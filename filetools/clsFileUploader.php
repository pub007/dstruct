...
/**
 * Whether to throw and error if there is no file uploaded.
 * @var unknown
...
public function __construct() {
	...
	foreach ($_FILES as $file) {
		if ($c == 0) {$this->defaultkey = key($_FILES);}
		if ($file['size'] != 0) {$this->totalfiles++;}
		$c++;
	}
}
 ...
 *@param integer $mode OVERWRITE_ Constant as above
 */
public function setOverwriteMode($mode) {$this->overwritemode = $mode;}
 ...
 *@param string $char
 */
public function setOverwriteChar($char) {$this->overwritechar = $char;}
 ...
 */
public function setFileRequired($bln) {$this->requirefile = $bln;}
 ...
 *@param string $name
 */
public function setNewName($name) {$this->newname = $name;}
...
public function setSavePath($path) {
	if (substr($path,strlen($path)-1,1) != '/') {$path .= '/';} // make sure path ends with '/'
	//echo $path;
 ...
 *@param array $mimes Allowable Mime-Types
 *@param array $extns Allowable file-extensions
 */
public function setAllowableTypes($mimes, $extns) {
 ...
 */
public function setUploadErrors($errors) {$this->uploaderrors = $errors;}
 ...
 *@return string
 */
public function getOverwriteChar() {return $this->overwritechar;}
 ...
 *@return string
 */
public function getSavePath() {return $this->savepath;}
 ...
 *@return string
 */
public function getDefault() {return $this->defaultkey;}
 ...
 *@return boolean
 */
public function getFileRequired() {return $this->requirefile;}
 ...
 *@return integer
 */
public function getOverwriteMode() {return $this->overwritemode;}
 ...
 *@return array
 */
public function getUploadErrors() {return $this->uploaderrors;}
 ...
 *@return boolean
 *@param string $key {@link setDefault()}
 */
public function isError($key = false) {
	$key = $this->validKey($key);
	// what if more errors added?...
	if ($_FILES[$key]['error'] > UPLOAD_ERR_EXTENSION) {return true;}
	if ($this->requirefile) {
...
public function getErrorMessage($key = false) {
	$key = $this->validKey($key);
	if ($_FILES[$key]['error'] > UPLOAD_ERR_EXTENSION) {return 'An unknown error occurred';}
	if ($this->isError()) {
...
public function isAllowableType($key = false) {
	$key = $this->validKey($key);
	if (in_array(strtolower($_FILES[$key]['type']), $this->mimetypes) == false) {return false;}
	if (in_array(strtolower($this->extension($key)), $this->extensions) == false) {return false;}
	return true;
}
 ...
 *@return integer
 */
public function count() {return $this->totalfiles;}
...
public function save($key = false) {
	...
	if (!move_uploaded_file($_FILES[$key]["tmp_name"], $this->savepath . $savename)) {return false;}
	return true;
}
...
private function validKey($key) {
	...
	if ($key) {
		...
		throw new DStructGeneralException('FileUploader::validKey() Property call on non-existent file');
	}
}
